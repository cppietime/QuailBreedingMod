...
public class BlobLeafPlacer extends GeneticLeafPlacer {
    public BlobLeafPlacer() {
        super("blob");
    }
    ...
    public Set<BlockPos> placeLeaves(WorldGenLevel level, GeneticFoliageAttachment attachment, GeneticTreeTileEntity geneEntity) {
        ...
        for (int axial = -2; axial < 3; axial++) {
            ...
            int range = axial < 1 ? 2 : 1;
            for (int i = -range; i < range + width; i++) {
                for (int j = -range; j < range + width; j++) {
                    if ( (i == -range || i == range + width - 1) &&
                            (j == -range || j == range + width - 1) &&
